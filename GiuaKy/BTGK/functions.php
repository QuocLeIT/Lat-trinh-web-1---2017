...
function AddBook($fullname, $price, $content, $imgFilename) {
  global $db;
  ...
  $stmt = $db->prepare("INSERT INTO Book (TenSach, imgDaTa, GiaBan, MoTa, iduser) VALUE (?, ?, ?, ?, ?)");
  $stmt->execute(array($fullname, $imgData, $price, $content, $_SESSION['userId']));
  return 1;
}

function AddTour($fullname, $matour, $imgFilename, $price, $songay, $socho, $danhgiadiem) {
  global $db;
  //mở file ảnh để đọc với chế độ đọc binary  
  $fh = fopen($imgFilename, "rb");  
  $imgData = fread($fh, filesize($imgFilename));  
  ...
  //$sql = "INSERT INTO tblImage (imgData) VALUES('" . mysql_real_escape_string($imgData, $conn) . "')";  
  //mysql_query($sql, $conn);  
  $stmt = $db->prepare("INSERT INTO tour (TenTour, MaTour, imgDaTa, Gia, Diem, SoNgay, SoCho) VALUE (?, ?, ?, ?, ?, ?, ?)");
  $stmt->execute(array($fullname, $matour, $imgData, $price, $danhgiadiem, $songay, $socho));
  return 1;
}
...
function getBooks() {
  global $db;
  $stmt = $db->prepare("SELECT TenSach, imgDaTa, GiaBan, MoTa from Book order by TenSach");
  $stmt->execute();
  $posts = $stmt->fetchAll(PDO::FETCH_ASSOC);
  return $posts;
}
function getTours() {
  global $db;
  $stmt = $db->prepare("SELECT TenTour, MaTour, imgDaTa, Gia, Diem, SoNgay, SoCho from tour order by TenTour");
  $stmt->execute();
  $posts = $stmt->fetchAll(PDO::FETCH_ASSOC);
  return $posts;
}
