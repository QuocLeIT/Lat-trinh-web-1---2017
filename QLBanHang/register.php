<?php 
  ...
  if (!empty($_POST['fullname']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['agree-tos']) && $_POST['agree-tos'] == 'on') {
    ...
    if ($user) {
      ...
      $insertId = createUser($fullname, $email, $password, $phone);
...
<form method="POST">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="phone" name="phone" placeholder="Số điện thoại" value="<?php echo isset($_POST['phone']) ? $_POST['phone'] : '' ?>">
  </div>
  <div class="form-group">
